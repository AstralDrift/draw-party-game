...
import { finalWinnerText, roundHighlightCards, roundOutcomeText, type RoundHighlightCard } from './polish';
import {
  defaultRoomSettings,
  phaseLabel,
  type RoomSettings,
...
const sessionToken = getStoredValue('draw-party-session-token', () => crypto.randomUUID());
...
function send(message: Parameters<GameSocket['send']>[0]): boolean {
...
function renderPlayerLobby(): HTMLElement {
  ...
  return el(
    ...
    el(
      ...
      el('p', { class: 'eyebrow' }, self ? `${self.name}, you're in` : "You're in"),
      ...
      el('p', { class: 'muted' }, 'Watch the TV. Your device becomes the controller when each round starts.')
    ),
...
function renderResults(result: RoundResult | null | undefined, includeDrawing: boolean): HTMLElement {
  ...
  const highlights = renderRoundHighlights(roundHighlightCards(result));
  ...
  return el(
    ...
    el('div', { class: 'result-sidebar' }, highlights, deltas, breakdown)
  );
}

function renderRoundHighlights(highlights: RoundHighlightCard[]): HTMLElement | null {
  if (highlights.length === 0) {
    return null;
  }
  return el(
    'div',
    { class: 'round-highlights', 'aria-label': 'Round highlights' },
    ...highlights.map((highlight, index) =>
      el(
        'article',
        { class: `highlight-card highlight-${highlight.tone}`, style: `--row-index:${index}` },
        el('span', { class: 'highlight-label' }, highlight.label),
        el('strong', { class: 'highlight-title' }, highlight.title),
        el('p', { class: 'highlight-detail' }, highlight.detail)
      )
    )
  );
}
