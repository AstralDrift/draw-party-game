...
test('one TV and three phones complete a full drawing round', async ({ baseURL, browser }) => {
  ...
  try {
    ...
    for (let turn = 0; turn < players.length; turn += 1) {
      ...
      await expect(tv.locator('.round-highlights')).toBeVisible();
      await expect(tv.locator('.highlight-card').first()).toBeVisible();
      await expectNoVerticalOverflow(tv);
      await expectLastResultRowVisible(tv);
...
async function expectLastResultRowVisible(page: Page): Promise<void> {
  await expect
    .poll(async () =>
      page.evaluate(() => {
        const panel = document.querySelector('.results-panel')?.getBoundingClientRect();
        const rows = Array.from(document.querySelectorAll('.breakdown-row'));
        const lastRow = rows.at(-1)?.getBoundingClientRect();
        if (!panel || !lastRow) {
          return false;
        }
        return lastRow.bottom <= panel.bottom + 2;
      })
    )
    .toBe(true);
}
