...
4. **Voting** — non-artists pick the real prompt; artist watches; reactions allowed. A normalized truth match is omitted from the fake list and becomes that player's locked correct vote. Normalized duplicate fakes share one option, and every coauthor is blocked from voting for it. If only the truth remains but a connected eligible player has not already locked a vote, the server still presents that one-option vote instead of awarding free points.
...
- A failed deadline-driven transition does not refresh room activity. A room with no connected display or players therefore remains eligible for normal TTL cleanup instead of being kept alive by maintenance retries.
