...
- The first connected phone is marked `isHost` and may change lobby settings, start the game, Continue results, or Play Again. If that host disconnects, the engine promotes another connected player. The TV display remains authorized too (optional remote / e2e), but party play should not require a TV remote after the room code appears.
