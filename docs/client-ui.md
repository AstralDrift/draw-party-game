...
2. **Drawing** — phone stroke pad + TV count of who is in; names still out sit under the count with no Waiting on coach line. The prompt owns the phone header; tools sit in the clock row; Submit appears after ink. Reconnect overlays the pad instead of hiding. Retry errors float above the pad without blocking Submit. Long gestures compact a bounded active buffer and keep capturing through pointer-up; completed strokes retain the existing 180-point limit.
...
Every phone lobby includes a collapsed “How to play” disclosure below its primary controls; it never blocks starting the game. Guessing uses a visible “Fake title” label and short placeholder. Anonymous TV voting and tally titles use consistent lowercase presentation; the reveal preserves original wording. Pending joins keep “Change room” available, and terminal session errors return an editable join form without replacing the player identity.
